...
void MainWindow::BtnPlotClick()
{
    ...
    QString file9=OutFile9->text();
    QString file[]={file1,file2,file3,file4,file5,file6,file7,file8,file9};
    QString cmd1="rtkplot_Qt",cmd2="..\\..\\..\\bin\\rtkplot_qt",opts=" -r";
    QCheckBox *cb[]={
        OutFileEna1,OutFileEna2,OutFileEna3,OutFileEna4,OutFileEna5,OutFileEna6,OutFileEna7,OutFileEna8,OutFileEna9
    };
    int i,ena[9];
    
    for (i=0;i<9;i++) ena[i]=cb[i]->isEnabled()&&cb[i]->isChecked();
...
{
    QString selectedFilter="SBAS Log (*.sbs)";
    OutFile8->setText(QDir::toNativeSeparators(QFileDialog::getOpenFileName(this,tr("Output SBAS/LEX Log File"),QString(),
              tr("All (*.*);;RINEX OBS (*.obs *.*O);;RINEX NAV (*.nav *.*N *.*P);;RINEX GNAV (*.gnav *.*G);;RINEX HNAV (*.hnav *.*H);;"
                 "RINEX QNAV (*.qnav *.*Q);;RINEX LNAV (*.lnav *.*L);;SBAS Log (*.sbs);;LEX Log (*.lex)"),&selectedFilter)));
}
// callback on button-output-file-7 -----------------------------------------
...
void MainWindow::UpdateEnable(void)
{
    ...
    OutFile7       ->setEnabled(OutFileEna7->isChecked()&&RnxVer<3&&(NavSys&SYS_CMP));
    OutFile8       ->setEnabled(OutFileEna8->isChecked()&&RnxVer<3&&(NavSys&SYS_IRN));
